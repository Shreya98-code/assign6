...
def main(args):
    ...
    #here args.train is redundant and we just need the file name to be imported from the s3 bucket. This change can be observed in sentiment dataset
    train_dataset = sentiment_dataset.train_input_fn(args.train, 'train.json', training_config)
    validation_dataset = sentiment_dataset.validation_input_fn(args.validation, 'dev.json', training_config)
    eval_dataset = sentiment_dataset.eval_input_fn(args.eval, 'eval.json', training_config)
